...
export async function adminSeedCompanyContent(companyId: string) {
  ...
  const color = (co.logo_color ?? '#7C3AED') as string
  ...
  if ((mc.count ?? 0) === 0) {
    const allMilestones = [
      { year: founded,      type: 'founding',    icon: '🚀', accent_color: '#7C3AED', bg_color: '#F5F3FF', title: `${name} founded`,                   detail: `${name} is established with a founding team of 6. The company sets out to reimagine ${category.toLowerCase()} infrastructure from the ground up, with its first office in ${hq}.`, badge: 'Origin' },
      ...
      { year: founded+8,    type: 'milestone',   icon: '🦄', accent_color: '#7C3AED', bg_color: '#F5F3FF', title: 'Achieves $1B Unicorn Valuation',       detail: `${name} joins the global unicorn club in a $180M Series D. The round values the company at $1.2B. More than 2,000 enterprises now use the platform across 34 countries.`, badge: 'Milestone' },
  ...
  if ((sc.count ?? 0) === 0) {
    await supabase.from('company_standards').insert([
      ...
      { company_id: companyId, sort_order: 6, code: 'WCAG 2.1 AA',     category: 'Accessibility', cat_color: '#8B5CF6', status: 'Compliant',   description: 'Web Content Accessibility Guidelines met across all customer-facing products.' },
  ...
  if ((dc.count ?? 0) === 0) {
    const deptDefs = [
      ...
      { id: 'security',    name: 'Security',          icon: '🛡️', color: '#8B5CF6', headcount: 28  },
      { id: 'product',     name: 'Product',           icon: '🎯', color: '#7C3AED', headcount: 85  },
