...
function runScraper(
  ...
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(process.cwd(), 'scripts', 'seed_roles.py')
    ...
    const timer = setTimeout(() => {
      child.kill('SIGTERM')
      reject(new Error('Roles scraper timed out after 90 seconds'))
    }, timeoutMs)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Roles scraper needs the auth token to query existing departments for linking
    const scriptArgs = [
      '--company',    name!,
      '--website',    website!,
      '--timeout',    '14',
    ...
    if (process.env.NODE_ENV === 'development' && stderr) {
      console.log('[seed-roles] scraper log:\n', stderr)
    }
    ...
    if (count === 0) {
      return NextResponse.json(
        { error: `No roles found for "${name}". Seed departments first so roles can be linked.` },
        { status: 404 },
      )
    }
    ...
    console.error('[seed-roles] error:', message)
    ...
    if (message.includes('timed out')) {
      return NextResponse.json(
        { error: 'Roles scraper timed out. Try again.' },
        { status: 504 },
      )
    }
